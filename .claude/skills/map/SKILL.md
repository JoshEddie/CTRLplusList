...
- **`PLOTTING`** (HITL — you plot the course) — run `/mattpocock-skills:grilling` on the question, decisions put to the owner one at a time. The agent never plots for the helm: a plotting session that answers its own questions has broken the contract. If the owner is not engaging live, the ticket stays open.
...
3. Run `/mattpocock-skills:grilling` to name the destination — it fixes the scope. Grill breadth-first to surface open decisions, then scale to what surfaced:
