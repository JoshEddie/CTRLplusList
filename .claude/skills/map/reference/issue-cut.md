...
- Body pre-distilled — problem, settled decisions linked from the map, constraints — and links the map issue so `/embark-start` inherits its Decisions so far.
...
- **Exception — the scout's closing e2e chunk:** the map-wide e2e coverage chunk cut on the e2e scout's recommendation is born `MUSTER`, not `CHARTED` — its plan (coverage rows plus deliberate skips) lives in the ticket body, it gets no OpenSpec change, and it enters work through `/embark-apply`'s MUSTER lane, never the departure arc's planning members.
