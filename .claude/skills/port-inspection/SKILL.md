...
2. **Subagent reads the map.** The `SCOUTING` subagent reads each sub-issue's `/landfall` summary comment plus the landed code. A sub-issue carrying **no** summary comment is not an error and not "no user-visible changes" — the subagent derives that chunk's user-visible changes from its `issue-<N>:` commits and archived change. It posts its report as the resolution comment and auto-closes with the ***unreviewed*** marker per the scouting mechanic.
