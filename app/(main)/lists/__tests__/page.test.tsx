...
vi.mock('../MyListsPage', () => ({
  default: () => <div data-testid="my-lists-stub" />,
}));

describe('Page', () => {
  it('Default_RendersListCollectionsMainWrappingMyListsPage', () => {
    render(<Page />);
    const main = screen.getByRole('main');
    expect(main).toHaveClass('container', 'container--list-collections');
    expect(main).toContainElement(screen.getByTestId('my-lists-stub'));
  });
  ...
  it('Suspense_FallbackIsPageLoadingIndicator', () => {
    const tree = Page() as {
      props: {
        children: {
          type: unknown;
          props: { fallback: { type: unknown; props: { size: string } } };
        };
      };
    };
