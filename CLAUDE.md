...
- **Five pre-merge gates, checked separately**: `npm run lint` (zero errors, zero non-size warnings) · `npx tsc --noEmit` · `npm run build` · `npm run test:coverage` · `npm run test:e2e`.
...
Project flow: `/opsx:propose` → `/opsx:apply` → `/opsx:archive` → `/finalize-spec-purposes`. The last step is a repo-owned skill, not part of the generated OpenSpec set: upstream archive/sync stubs `TBD` Purposes onto newly created capability specs, and the skill repairs them (and ratchets down the `KNOWN_TBD` baseline in [scripts/check-spec-purposes.mjs](scripts/check-spec-purposes.mjs), the lint gate that blocks new stubs).
