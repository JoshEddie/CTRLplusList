...
3. **`/set-sail`** — the apply wrapper: enforces one-change-mid-apply at the moment the tree becomes occupied, flips `CHARTED` → `UNDER SAIL`, states the mid-voyage disciplines (discoveries logged as rich `OFF THE MAP` issues, never charted mid-voyage; a mirage stops work and fires `/anchor`), delegates to `/opsx:apply`.
...
6. **`/landfall`** — gates (latest review-round verdict `clear to land`, tasks all `[x]`, `openspec validate --strict`, local lint + typecheck), then one owner question: dev verification before sealing? **Fast path** (no): two signed commits (`issue-<N>:` work, `issue-<N>: archive <change>`), one push, no CI wait. **Verified path** (yes): push the work commit, green CI + owner's live click-test, then seal. Either path: `/finalize-spec-purposes` runs before the seal commit is staged, every hand-off carries the paste-ready commit message, bookkeeping is eager (milestone + `IN PORT` — **never closes the issue**). Red CI → fix forward under the same prefix. State-driven and self-healing: resumes from repo state, sweeps leftover bookkeeping.
7. **`/anchor`** — all bearing moves: promote (fog → typed ticket), demote (mirage — reopen the *original* ticket, evidence comment, gist marked *reopened*, affected chunks coin-flipped `UNCHARTED`), map-body re-sync, and mid-voyage triage by blast radius (patch at sea / park to `adrift/issue-<N>` + `ADRIFT` / discard + `UNCHARTED`). Half-finished work never merges to `dev`. Any session may anchor at the moment of discovery; none carries a detection duty.
8. **`/close-map`** — the inspection batch-point and the only closer: walks open `IN PORT` chunks with the owner (verified live → close), closes the map only when every chunk is closed.
