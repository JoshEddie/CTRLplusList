...
Per `testing-foundation` colocation + the `__tests__/` convention: `app/(main)/lists/bookmarks/__tests__/{page,BookmarksPage,BookmarksList}.test.tsx`, `app/(main)/lists/history/__tests__/{page,HistoryPage,HistoryList}.test.tsx` (joining the existing `HistoryActions.test.tsx` / `HistoryCard.test.tsx` from 4.14 in the same dir), and `app/(main)/purchased/__tests__/{page,loading}.test.tsx`. The three `page.test.tsx` basenames live in distinct `__tests__/` directories — no collision (vitest keys on the full path). Shared Arrange (a `makeSession` / `makeViewer` factory and the `redirect()` sentinel) is hoisted into each file's `beforeEach`; a colocated `__tests__/test-helpers.tsx` is extracted **per directory** only on 3+ reuse (the established threshold). The three async shells live in three different directories (one per dir), so per-directory 3+ reuse is unlikely — inline `beforeEach` Arrange is the expected disposition, recorded by the duplication audit.
