...
- **New files:** 8 `.test.tsx` files — 3 under `app/(main)/lists/bookmarks/__tests__/` (`page`, `BookmarksPage`, `BookmarksList`), 3 under `app/(main)/lists/history/__tests__/` (`page`, `HistoryPage`, `HistoryList` — joining the existing `HistoryActions.test.tsx` / `HistoryCard.test.tsx` from 4.14), and 2 under `app/(main)/purchased/__tests__/` (`page`, `loading`), plus 2 colocated `__tests__/test-helpers.tsx` fixtures (history + bookmarks). Shared Arrange (an `auth()`-mock / session+viewer factory and the `redirect()` sentinel) is hoisted into each file's `beforeEach`; the typed `makeRow` row-fixture is extracted to the per-directory `test-helpers.tsx` under CLAUDE.md's weight·drift·count duplication judgment.
