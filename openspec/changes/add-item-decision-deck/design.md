...
### D10 — Timeout screen for hard failures; rate-limit + manual unchanged
A hard fetch failure/timeout shows a dedicated **Timeout** screen ("That link wouldn't load" / try a different link / build it by hand), replacing the inline notice. 429/`rate_limited` still returns to URL entry with the slow-down message; "build it by hand" routes to the blank Preview. This refines `product-link-prefill`'s failure requirement (manual fallback is reachable from the Timeout screen rather than auto-rendered).
