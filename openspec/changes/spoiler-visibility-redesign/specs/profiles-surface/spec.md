...
**The Settings panel SHALL additionally carry claim visibility**, in two parts: the profile-level **default** that seeds the baseline of every membership created from then on, and each current member's own **baseline**, each expressed as a single spoiler tier (`spoiler-visibility`) settable on every one. Claim visibility is a setting rather than a permission, so it belongs here and not beside the roster in Permissions, which carries roles, admission and removal only; mixing what a member is allowed to do with what they have asked to see would make both harder to read. This is the decision recorded in `2026-08-31-a-member-preference-is-a-setting-not-a-permission`.

These controls are governed per control rather than per panel. The profile-level default SHALL take the `owner` floor. A member's own baseline SHALL be editable by that member whatever their role, and by an owner. A `manager` therefore sees the name-and-tagline form disabled, every other member's baseline disabled, and their own baseline **enabled** — the one control in the panel their role does not forbid. Enforcement never rests on the disabled control: a member who submits another's baseline by any other means is refused by the action.

Changing the profile-level default SHALL NOT alter the baseline of any account already holding a membership; those change one row at a time. The panel SHALL therefore not present the default as a state of the members' rows.

Where the viewer's role on the profile is `manager`, every other control in the space that their role forbids SHALL render in a **disabled** state rather than be omitted, so the space reads as one they lack the right to use rather than as one without the feature. The Settings form SHALL render with every field disabled and its submit control present but disabled, and the identity header's avatar SHALL carry a disabled edit affordance.
...
#### Scenario: A manager can still set their own claim visibility

- **WHEN** a viewer holding `manager` opens the profile's space
- **THEN** their own baseline control renders enabled
- **AND** the profile-level default and every other member's baseline render disabled

#### Scenario: An owner sets another member's baseline

- **WHEN** a viewer holding `owner` changes another member's baseline in the Settings panel
- **THEN** that membership's stored baseline is updated

#### Scenario: The tier is settable on every baseline

- **WHEN** the Settings panel renders a member's baseline
- **THEN** its spoiler tier is settable as one of the four stages

#### Scenario: Changing the default leaves members where they are

- **WHEN** an owner changes the profile-level default while members already exist
- **THEN** no member's baseline changes
...
- **AND** their own claim visibility is settable from the Settings panel
