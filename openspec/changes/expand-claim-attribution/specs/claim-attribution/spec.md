...
With spoilers disabled, the owner's view of their own list SHALL show no claim information and no claim or unclaim affordances (unchanged from today). With spoilers enabled, the owner SHALL additionally see a claim affordance on items with remaining claimable quantity, opening the same purchase modal (self-purchase or someone-else step), and an unclaim affordance on existing claims. The spoiler gate is a UI surface only — server-side authorization for owner claims and master unclaim derives from item ownership, not from any spoiler parameter. Owner claims SHALL count toward `quantity_limit` identically to viewer claims.
...
- **THEN** a claim affordance renders and opens the purchase modal with the self / someone-else choice
