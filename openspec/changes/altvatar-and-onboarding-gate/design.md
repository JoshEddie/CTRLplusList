...
- **`resolveIdentity` already returns `null` when an account holds no `self` membership**, so a profile-less account can already write nothing. The gate's structural enforcement is a property the code has, not one this change invents — it only removes the thing that was preventing profile-less accounts from existing.
