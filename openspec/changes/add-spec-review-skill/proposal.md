...
- The skill is **self-contained** — it does not sub-call or take a runtime dependency on the external `engineering:code-review` plugin. The standard review dimensions (security, performance, correctness, maintainability) are inlined as a sub-agent brief, with a one-line note crediting their lineage.
...
- Reads the **active** change directory `openspec/changes/<name>/` for an auto-detected change (never substituting an `archive/` copy), because the skill runs before the archive step. Exception: when auto-detection finds nothing and the user explicitly names an already-archived change, it reads from `openspec/changes/archive/*-<name>/` and skips the (active-only) `openspec validate`.
