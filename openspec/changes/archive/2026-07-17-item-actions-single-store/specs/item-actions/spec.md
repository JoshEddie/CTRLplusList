...
The `item-actions` capability owns the per-item action area: the state matrix deciding which actions render for a given viewer, claim state, and spoiler state; the primary-slot + two-up-secondary layout; the external-link `↗` affordance; the non-button `Fully claimed` status; and the view-only mode preview surfaces consume. Claim semantics (what the modal does) belong to `claim-attribution`; store validity and the price line belong to `item-store-links`.
