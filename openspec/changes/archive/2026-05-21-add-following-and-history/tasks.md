...
- [x] 14.1 Manual: log in as User A, open User B's `'public'` list → row appears in A's `/lists/history` with non-bookmarked styling.
- [ ] 14.2 Manual: refresh the list page → `visit_count` increments (verify in DB); single row in history.
- [x] 14.3 Manual: click Bookmark on the list page → row gains 🔖 indicator in history and appears in `/lists/bookmarks`.
- [ ] 14.4 Manual: click Unbookmark → row remains in history with bookmark removed.
- [x] 14.5 Manual: User A's own list, opened by User A → no visit row recorded; Follow + Bookmark buttons not shown.
...
- [x] 14.12 Manual: each home-page rail collapses/expands; state persists across reload.
- [x] 14.13 Manual: "Clear non-bookmarked" wipes history but preserves bookmarks; "Clear all" wipes both.
- [ ] 14.14 Manual: unauthenticated user views a `'public'` list → no visit recorded; CTAs not shown.
- [x] 14.15 `pnpm tsc --noEmit` and `pnpm lint` clean.
...
- [x] 15.6 ~~superseded by colocate-follow-with-owner-name~~ In `app/(main)/users/ui/components/FollowButton.tsx`, render an inline `.follow-disclosure` note under the button when the viewer is not yet following. Hide it once `following === true`.
- [x] 15.7 (partially ~~superseded by colocate-follow-with-owner-name~~ — `.follow-button-wrap` / `.follow-disclosure` were removed; `.connections-row-meta` / `.connections-row-since` retained) CSS additions: `.connections-row-meta` / `.connections-row-since`, `.follow-button-wrap` / `.follow-disclosure`. Reuse existing tokens (`--muted-text-color`, `--background-color`).
...
- [x] 16.9 ~~superseded by colocate-follow-with-owner-name~~ Manual: Follow button on a list page renders the inline disclosure when not-following; disclosure disappears after clicking Follow.
- [x] 16.10 `pnpm tsc --noEmit` and `pnpm lint` clean.
