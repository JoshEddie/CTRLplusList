...
`MainShell` is a client component (`'use client'`, uses `usePathname()` for variant classes), which means `loading.tsx` auto-injection lands _inside_ the client boundary as a children prop — not in the server tree where Next 16 needs the Suspense to split the static shell from the dynamic stream. Therefore neither (a) the existing `app/(main)/lists/[id]/loading.tsx`, (b) the `app/(main)/items/loading.tsx`, nor (c) the existing async `app/(main)/items/[id]/layout.tsx` Suspense satisfy `cacheComponents`. The Suspense has to live in the **page.tsx** itself (synchronous server component, render-tree position above any `await`).
