...
Fresh-context review of a MUSTER (tests-only) diff: one testing-arena agent reading the bundled arena T brief checks test substance and traceability against the cited active-spec scenarios — guarding against "tests pass, so they're good" — and reports its verdict in the session, where `/landfall`'s MUSTER branch reads it via owner confirmation as the landing gate.
