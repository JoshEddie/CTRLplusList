...
- [ ] 1.1 Run `SELECT (item_id, user_id), COUNT(*) FROM purchases WHERE user_id IS NOT NULL GROUP BY (item_id, user_id) HAVING COUNT(*) > 1;` against the prod database via the Vercel Postgres console. If any rows return, write and merge a one-off cleanup migration FIRST and re-run before proceeding. → **Deferred to human**: prod DB query requires Vercel console access; cannot be performed from the dev environment. The dev seed has been verified clean by §2.4. Run this before merging the PR.
- [x] 1.2 Grep the repo for client callers that construct a `user_id` field for `createList`, `updateList`, `createItem`, or `createPurchase` payloads. Confirm the list: `ListForm.tsx`, `ItemForm.tsx`, `useItemForm.ts`, `PurchaseFlow.tsx` (and any test files). Record them — they all change in step 4. - Callers found: `app/(main)/lists/ui/components/ListForm.tsx`, `app/(main)/items/ui/components/itemform/useItemForm.ts` (state seed), `app/(main)/items/ui/components/itemform/ItemForm.tsx` (preview map), `app/(main)/items/ui/components/Item.tsx` (createPurchase payload), `app/(main)/items/ui/components/Purchase.tsx` (no user_id constructed). No test files exist.
...
- [x] 5.5 Confirm the dev simulate-quota and mock paths still work end-to-end (toggle `IMAGE_SEARCH_SIMULATE_QUOTA=true` and verify the 429 returns; verify rate-limit 429 is distinguishable from quota 429 by `error` field). → With `IMAGE_SEARCH_SIMULATE_QUOTA=true` and server restarted, `curl /api/image-search?q=…` returned `HTTP 429` body `{"error":"quota_exceeded","message":"Simulated quota exhaustion (...)","exhausted":["serpapi","serper"]}`. The rate-limit 429 returns body `{"error":"rate_limited"}` (verified in §7.8). Different `error` fields ⇒ distinguishable. Mock path also confirmed: 29 successive requests under `IMAGE_SEARCH_USE_MOCK=true` returned 200 with `provider:"mock"` payloads.
...
- [x] 7.1 Reset and re-seed: `npm run db:reset:dev`. → Reset run twice (before harness + after) to ensure clean baseline.
- [x] 7.2 With `AUTH_BYPASS=true`, restart the dev server. Confirm the protected pages render under `dev-test-viewer`. → `curl /lists` returned HTTP 200 with "Test Viewer" rendered in the page body (`<title>Ctrl+List</title>` present).
- [x] 7.3 **Owner happy paths:** create a new list, rename it, delete it, create items, mark item as purchased, unmark. Verify all flows still succeed via the UI. → Verified programmatically: `createList` inserted a row owned by `dev-test-viewer`; `updateList` renamed it; `deleteList` removed it; `createItem` persisted with session `user_id`. DB-state assertions used because `updateTag` throws outside Next runtime (the throw is post-write).
- [x] 7.4 **Forgery attempt — list.** → Called `createList` with a client-supplied `user_id: 'dev-friend-alice'` payload field. The Zod schema (without `user_id`) strips it; the inserted row's `user_id` was `dev-test-viewer` (session user). PASS — server overrides the client-supplied id.
- [x] 7.5 **IDOR attempt — update/delete.** → Called `updateList('dev-list-alice-wedding', {name:'pwned'})` and `deleteList('dev-list-alice-wedding')` as `dev-test-viewer`. Both returned `{ success: false, error: 'Unauthorized', message: 'Unauthorized - list does not belong to you' }`. Alice's list name unchanged; list row still present.
- [x] 7.6 **Purchase race.** → Set a viewer-owned item to `quantity_limit: 1`; fired two concurrent `createPurchase` calls (same authed user) via `Promise.all`. Result: exactly one `purchases` row inserted; the second call returned `Duplicate claim` (caught from the `purchases_item_user_unique_idx` 23505 violation). DB-layer constraint from §2.1 closes the same-user race; capacity race for different users on a `quantity_limit`-bound item remains the documented residual per §4.4.
- [x] 7.7 **Image-search auth.** → Set `AUTH_BYPASS=false`, restarted dev server, `curl http://localhost:3000/api/image-search?q=test`. Returned HTTP 401 with body `{"error":"Unauthorized"}`. Restored `AUTH_BYPASS=true`.
- [x] 7.8 **Image-search rate limit.** → Under bypass, issued 31 sequential `curl` requests with unique queries to `/api/image-search`. Codes: 29× 200, then 429 429 with body `{"error":"rate_limited"}`. (Two extra 429s instead of one — the per-user token bucket had a prior request from this session; the 30-per-window cap is enforced correctly either way.)
