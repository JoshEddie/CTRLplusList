...
## 5c. Folded-in CI/harness hardening — getting the suite green in CI

Running the suite in CI surfaced harness/CI work beyond the §5b product fixes. By owner decision it was folded in here (mirroring §5b) rather than spun out — each item is small and either unblocks the CI e2e run or is low-risk hygiene. This is operational hardening of the **existing** jobs, NOT a reshape of the e2e *execution model* (tiers, two-server, build-once stay 6.0's). Rationale — esp. the type-gate decision — is in [design.md](design.md) Decision 8.

- [x] 5c.1 Make e2e reruns deterministic — reset the DB to the canonical fixture before the suite. This one is an execution-model property, so it was elevated to a **6.0 SHALL**, not recorded only here: [test-e2e-foundation](../test-e2e-foundation/tasks.md) task 3.5, design Decision 6, and two `testing-foundation` scenarios. `setup-e2e-db.sh` stays schema-only; the reset lives in the `test:e2e` caller so it can never wipe the `dev:local` developer DB.
...
- [x] 5c.7 Repoint the VS Code debug config ([.claude/launch.json](../../../.claude/launch.json)) from `npm run dev` (port 3000) to `npm run dev:local` (port 4000), and `web-prod` to port 4001 — so the in-editor "Run" launches the local-mode bypass server this work depends on, clear of the default dev ports. Editor tooling only; no app/runtime effect.
...
- [x] 6.3 Four-audit findings recorded below. NO `vitest.config.ts` threshold change; NO e2e *execution-model* reshape (owned by 6.0) — the CI/harness *hardening* folded in here (§5c) is operational, not a model change.
...
- [x] 7.3 `npm run build` completes successfully (build/bundle errors only). _Type-correctness is NOT gated here — builds set `next.config.ts` `ignoreBuildErrors: true` — but by 7.2 / the CI `typecheck` job, which now runs `next typegen` + `tsc --noEmit` so route types are covered (§5c.3, design Decision 8). The build still runs on every PR via the e2e tier + on Vercel._
