...
Each seeder owns the id scheme it writes, so there are two homes rather than one: `test/helpers/profile.ts` for the fixture side, where the helper is the contract between `seedUsers` and the files reading what it inserts, and a local declaration in `scripts/seed-dev-users.ts` alongside the `friendId` and `KIDDO_PROFILE_ID` factories it already owns. An operational module importing from `test/` would be the repo's only `scripts/` → `test/` edge, and it holds only while that fixture module stays free of value imports — a constraint nothing enforces. Two copies of one pure line clear CLAUDE.md's trivial-inline bar on weight, drift hazard and count, and they serve disjoint data.
...
Four `profiles-data-model` requirements are removed as migration mechanics; four surviving behaviors are re-added under new names.

The fourth is a split rather than a rescue: the opaque-profile-id rule leaves the MODIFIED profile-identity requirement, which keeps profile-account independence only, and becomes an ADDED requirement of its own carrying the id-minting scenario. It is normative about the production minting paths — `createSelfProfile` and the `0010` backfill — and carves out the dev seed and test fixtures, whose deterministic `self-<account id>` ids D7 deliberately preserves. Without the carve-out the requirement reads as falsifying the helper D7 keeps.

Four more requirements are falsified without being migration mechanics, so each is rewritten in place rather than removed: `profiles-data-model`'s dev-seed requirement (its `db:reset:dev` wipe reaches profiles by `profiles.user_id`, leaving membership the sole handle once the column goes), `following`'s no-transactions requirement (two scenarios name the `profiles` lookup in `blockUser`'s statement sequence), and `claim-attribution`'s claim-modal and self-claim-suppression requirements (three normative references to the dropped `purchases.claimed_by`, plus the pool-read tag list D9 extends). One of those requirements carries a clause that fails today's bar without this change falsifying it: the pool read's `lib/data/user.ts` home, which has since moved to `lib/data/profile.ts`. It is corrected here rather than left to `spec-hygiene`. This change already rewrites that same sentence twice — `claimed_by` → `claimed_by_profile_id`, and the tag list gaining `profile_members` — so the clause is being reproduced either way, and reproducing it unchanged would ship a knowingly false SHALL naming a module the code does not use.
...
3. Add the drop migration: the seven remaining superseded columns, their indexes, and `purchases_item_user_unique_idx`. The eighth, `profiles.user_id`, is never created — step 1 rewrites `0010` in place.
