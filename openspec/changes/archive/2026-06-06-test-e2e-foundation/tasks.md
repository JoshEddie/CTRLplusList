...
- [x] 3.3 Add a setup/run helper script that: checks Docker (auto-start Docker Desktop on macOS, clear failure otherwise), brings up the sidecar, waits until healthy, and applies schema via `drizzle-kit push`. Refuse to run if `DATABASE_URL` resolves to a hosted DB (mirror `budget_eddiefamily/scripts/docker-setup-e2e-db.sh`). _(The data-state step — seed vs reset — is the caller's, not the shared helper's; see 3.5.)_
...
- [x] 3.5 Make the e2e run deterministic: keep the data-state step OUT of the shared `setup-e2e-db.sh` (now Docker + schema only) and put it in each caller — `dev:local` seeds (`db:seed:dev`, preserves UI-created rows), `test:e2e` runs `db:reset:dev` (cascade wipe + reseed) before the suite so every run starts from byte-identical state. _(Closes the non-hermetic-rerun gap surfaced reviewing `test-e2e-critical-flows`: defensive selection isolates tests within a run, but a persisted DB drains single-claim inventory across reruns; the reset makes runs reproducible. Splitting the step per-caller — not flagging the shared script — keeps a reset from ever wiping the `dev:local` developer DB.)_
