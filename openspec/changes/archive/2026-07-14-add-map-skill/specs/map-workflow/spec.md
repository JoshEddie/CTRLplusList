...
All work SHALL enter through `/map`: it accepts plain text or an existing GitHub issue and compiles it toward ready-to-work issues through three phases — **chart** (name the destination via `/mattpocock-skills:grilling`, grill breadth-first, create the map and its tickets), **work** (resolve tickets, scouting excepted in parallel), and **exit** (cut the destination into implementation chunks). No other skill SHALL create issues cleared for work, and the retired explore route SHALL NOT be routed to; map's charting is the one fog engine, and it SHALL preserve the explore write-back discipline — every issue map emits carries a distilled body that is the complete, current statement of what to build, approved by the owner before any issue is created or edited. Persistence machinery SHALL materialize only when state must outlive the session: a question answered inline never becomes a ticket, and the map issue exists only when unresolved decisions must cross a session boundary or the work must split — input that charts to one-change size SHALL compile to a single distilled `CHARTED` issue with no map artifact. Input SHALL be consumed, never converted: a prompt issue is closed with a pointer comment to what map created. Nothing SHALL ever be implemented from the map; every ticket resolves a decision, not a slice of the build.

#### Scenario: Small clear idea compiles without a map
- **WHEN** charting finds no fog and the idea is sized for one OpenSpec change
- **THEN** `/map` emits a single distilled issue labeled `CHARTED`, closes a prompt issue with a pointer comment, and creates no map
...
`/close-map` SHALL be the inspection batch-point and the only skill that closes a map: it walks each open `IN PORT` chunk with the owner (verified on the live deployment? → close), then closes the map when — and only when — every implementation chunk is closed. A map whose last chunk is merely `IN PORT` SHALL NOT close: `IN PORT` is uninspected cargo. Landfall labels; inspection closes.

#### Scenario: In-port chunks block the close
- **WHEN** `/close-map` runs while two chunks sit `IN PORT`
- **THEN** it prompts the owner to verify each (closing those confirmed) and closes the map only if none remain open

### Requirement: The definition layer SHALL only write GitHub issues and SHALL run ungated

`/map`, `/anchor` (its map-side moves), and `/close-map` SHALL limit side effects to GitHub issue operations (issues, comments, labels, sub-issue links, blocked-by relationships) via `gh`. They SHALL never mutate the working tree, never stage, never run `git commit`, and never push — with the sole exception of anchor's park move, which stages a WIP commit for the owner's signature and never signs it. They SHALL have no trunk preconditions gate — branch and tree state are irrelevant to issue writes.
