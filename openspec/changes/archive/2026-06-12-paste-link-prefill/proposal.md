...
- **New server endpoint** `POST /api/product-fetch` behind a `ProductFetcher` seam: tier 1 free server-side fetch + schema.org JSON-LD / OpenGraph parse; tier 2 Zyte API (`product: true`, `extractFrom: httpResponseBody`) for resistant sites (Amazon); app-side abort timeout (~15–20s) with route `maxDuration` above it; tier 3 is the manual-entry fallback.
