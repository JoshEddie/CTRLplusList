...
On success: map product → `useItemForm` initial values (name, imageUrl, one store row `{name: store, price, link: pastedUrl}` + provenance fields held in hook state — Description stays empty per the product-link-prefill spec) and a "Fetched from {store}" badge above the form. Prefilled values are validated eagerly: client-side name bounds mirror `ItemSchema` (3–100 chars) and the shell's submit button is disabled while the form is invalid, so an over-long fetched title is visibly blocked rather than failing at submit (applies to manual/edit flows too — one validation path). The phase machine lives in the container so phase 2 can lift it out to drive multiple pending cards.
