...
| `app/ui/components/TooltipWrapper.tsx` | 20 | Default-export function component; renders a `<div class="tooltip-container[ <className>]">` (base token alone when `className` is falsy; single-space join when truthy — no trailing whitespace either way) wrapping children + an optional `<span class="tooltip">` (gated by `showTooltip`, default true). | jsdom + RTL |
...
1. **Wrapper composition.** `<TooltipWrapper>` SHALL render an outer `<div>` whose className is `'tooltip-container'` when `className` is falsy and `` `tooltip-container ${className}` `` when truthy — exact-string in both branches, with NO trailing whitespace. The source uses a conditional `className ? `tooltip-container ${className}` : 'tooltip-container'` to avoid the trailing-space wart the prior template literal `` `tooltip-container ${className || ''}` `` emitted. The children prop SHALL render as the first child. Non-obvious (the conditional join is the right shape for a className composer — most contributors writing this for the first time reach for the template literal and produce the trailing-space wart); survives reimplementation only if tests lock both branches' exact strings; protects against silent regression where the wart returns and downstream consumers concatenating into the className inherit the extra space.
...
- `NoClassName_WrapperClassIsBaseOnly` (locks Decision 3b's exact-string contract via `expect(div.className).toBe('tooltip-container')` — no trailing space)
- `WithClassName_AppendedAfterSingleSpace` (`expect(div.className).toBe('tooltip-container foo')` for `className="foo"`)
- `EmptyStringClassName_BehavesLikeUndefined` (the falsy guard collapses `''` to the no-prop branch; result is `'tooltip-container'`)
...
- `TooltipWrapper.tsx` — the `className ? ... : ...` conditional join (replaced the prior `className || ''` short-circuit during this carve-out to trim the trailing-space wart). Disposition: **(a)** — `NoClassName_WrapperClassIsBaseOnly`, `WithClassName_AppendedAfterSingleSpace`, AND `EmptyStringClassName_BehavesLikeUndefined` tests cover both branches.
...
- **The `TooltipWrapper` class composition was trimmed in this carve-out** (from the template literal `` `tooltip-container ${className || ''}` `` to a conditional join). The original emitted a literal trailing space whenever no `className` was passed — a real wart. Risk of trimming during a coverage carve-out: bundling a behavior change with a test-only change muddies the diff. → Mitigation: the trim is a one-character source change driven by review feedback during the carve-out (the "lock the wart vs. fix the wart" call); both branches' exact-string output (`'tooltip-container'` / `'tooltip-container foo'`) are locked by the new tests, so any future regression-to-wart surfaces as a failing test with a clear name (`NoClassName_WrapperClassIsBaseOnly`). No existing consumer relied on the trailing space (verified by grep: no `.tooltip-container ` selector with significant trailing whitespace anywhere in `app/`).
