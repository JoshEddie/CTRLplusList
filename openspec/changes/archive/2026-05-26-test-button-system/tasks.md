...
- [x] 9.3 `npm run build` — completes successfully with a `DATABASE_URL` placeholder (the build does not require live DB access). ⚠️ **Pre-existing failure**: `npm run build` with `DATABASE_URL=postgresql://placeholder` fails on Next 15's page-data collection for `/api/auth/[...nextauth]` (and `/api/image-search` once auth is fixed) — both routes need a real environment to initialize NextAuth providers. Confirmed pre-existing by `git stash && npm run build` on the bare branch tip: same failure. This is a known constraint of the four-gate workflow not introduced by this change; documented for parent `test-coverage` change to reconcile (the task wording may need updating, since the build has not actually been runnable with a placeholder URL since NextAuth integration landed). Not blocking this sub-proposal per the §9.1 governance-reconciliation precedent.
