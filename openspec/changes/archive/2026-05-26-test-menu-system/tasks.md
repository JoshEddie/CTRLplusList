...
- [x] 7.4 Coverage report after the suite landed at **100% statements / 100% branches / 100% functions / 100% lines** across all five carve-out files. Dispositions per branch the v8 reporter initially flagged:
  ...
  - `Menu.tsx` line 68 (inside `focusAt`) `if (items.length === 0) return;`: **(c)** `/* v8 ignore next */` — `focusAt` is only invoked from the `onKey` branches, all of which sit AFTER the outer `if (items.length === 0) return;` guard, so the inner check is unreachable.
  - `Menu.tsx` line 77 `active ? items.indexOf(active) : -1` (falsy `active` branch): **(c)** `/* v8 ignore next */` — jsdom never resolves `document.activeElement` to `null`; falls back to `document.body`. The `: -1` fallback exists for non-DOM environments (SSR / strict isolation) and remains in source for defense in depth.
