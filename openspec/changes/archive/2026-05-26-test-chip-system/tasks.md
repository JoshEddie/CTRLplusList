...
- [x] 7.5 **Invariant-elevation audit** — per testing-foundation Requirement: "Sub-proposals SHALL elevate non-trivial invariants to capability-spec SHALLs". Walk every invariant the new tests enforce. Apply the three-part gate (non-obvious / survives reimplementation / protects real failure mode). Expected outcome: **multiple elevations** — this carve-out CREATES the `chip-system` spec. The elevated SHALLs in `specs/chip-system/spec.md` are:
  ...
  - Relocation: "Removable chips share the button focus/touch contract" (moved from `button-system`). The four scenarios are verbatim; the requirement description is corrected from `consume buttonClasses() internals` to `consume the same --btn-* token surface used by the button system` (chip uses CSS custom properties, not the `buttonClasses()` function — see design Decision 1). NOT a new elevation; preserves existing SHALL ownership.
...
- [x] 8.2 Confirm the `specs/chip-system/spec.md` delta contains the relocated requirement AS THE FIRST REQUIREMENT in the new spec: the four scenarios verbatim (zero text drift between archived `button-system` and active `chip-system`), AND the requirement description corrected from `consume buttonClasses() internals` to `consume the same --btn-* token surface used by the button system` per design Decision 1 (the original wording was a docs artifact `chipClasses.ts` never honored). The Migration note in the `specs/button-system/spec.md` REMOVED delta documents the correction.
- [x] 8.3 Confirm the `specs/testing-foundation/spec.md` delta is sub-proposal-archive-only per `test-coverage` design D13 (two-tier rollup): it records the chip-system carve-out bookkeeping (universal `COVERAGE_FLOOR`, `sonarjs/cognitive-complexity` promotion, `__tests__/` colocation, no per-file exclude for `index.ts` — already covered by existing glob) parallel to the test-button-system precedent. Confirm this delta is NOT added to `openspec/changes/test-coverage/specs/testing-foundation/spec.md` (the parent accumulator) and that `openspec/specs/testing-foundation/spec.md` (the active spec) is NOT created or modified by this sub-proposal — that file is created at `test-coverage`'s eventual archive from the parent accumulator.
