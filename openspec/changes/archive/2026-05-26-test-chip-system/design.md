...
- `button-system` — currently owns the chip primitive's "focus / touch contract" requirement. This proposal moves that requirement to the new `chip-system` spec (the four scenarios verbatim; the requirement description corrected to match the implementation — see Decision 1) and removes it from `button-system`. The `sm`-size opt-out requirement (which references chips as a valid context for `<Button size="sm">`) stays in `button-system` — it governs button sizing, not the chip primitive.
...
- Create the `chip-system` capability spec, migrating the chip requirement out of `button-system` (four scenarios verbatim; description corrected to match the implementation per Decision 1) and adding elevated SHALLs for the call-time contract (label derivation, stopPropagation, type=button, disabled passthrough, two-interactive-concerns DOM shape).
...
### Decision 1: Create a dedicated `chip-system` spec; move the existing chip requirement out of `button-system` (scenarios verbatim; description corrected to match the implementation).
...
The existing `button-system` requirement ("Removable chips share the button focus/touch contract via `<Chip>` primitive") and its four scenarios MOVE into `chip-system` as follows: the four scenarios are preserved verbatim (zero text drift); the requirement *description* is corrected from `"<Chip>" MUST consume buttonClasses() internals` to `"<Chip>" MUST consume the same --btn-* token surface used by the button system`. The original wording was a docs artifact inherited from `standardize-buttons` that the chip code never honored — `chipClasses.ts` is a standalone composer (`['chip', extra].filter(Boolean).join(' ')`); it does not call `buttonClasses()`. The two surfaces share CSS custom properties, not a function. The correction is a docs/code reconciliation, NOT a behavior change. The `button-system` spec delta REMOVES the requirement.
...
- **Moving the chip requirement out of `button-system` could cause a future reader to look for chip behavior in the wrong spec.** → Mitigation: the chip-system spec carries the four scenarios verbatim (with the description corrected per Decision 1). A grep for "chip" in `openspec/specs/` after the move surfaces the chip-system spec immediately. The `button-system` spec is not chip-aware after the move; no orphan reference is left behind.
...
- **This is the first carve-out that creates a brand-new capability spec while testing.** → Mitigation: the spec content is migration-from-button-system (no behavior change — scenarios verbatim, description corrected per Decision 1) plus elevation of existing-in-code SHALLs. The risk surface is "did the move preserve the four scenarios exactly?" — verifiable by diffing the scenarios between `button-system.md` (before) and `chip-system.md` (after).
