...
Today the only way to stay connected to another user's lists is to manually save each list one at a time (`saved_lists`). For a family/occasion-driven app where the *same* person produces a fresh list every cycle (Grandma's wishlist every birthday, Sis's Christmas list every December), this fights the grain of the data: you re-save the same person's lists year after year. The user-facing primitive should be **follow a person**, not **bookmark a list**.
