...
- **Modified source:** `lib/auth.ts` — the `signIn`/`jwt`/`session` callbacks extracted to named exports (testability refactor inside the carve-out; behavior preserved). `lib/dal.ts` — a small behavior-preserving simplification: four redundant nullish/optional operators that TypeScript already proves dead (non-nullable Drizzle relations / `COUNT(*)` aggregate; an over-wide `sanitizePurchases` param) were removed so the whole-file branch floor is met by real code, not `/* v8 ignore */` (see tasks §2.15). `tsc`-verified equivalent; no behavior change; **this change adds no `/* v8 ignore */`**.
