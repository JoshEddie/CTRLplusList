...
- **WHEN** `https://mock.test/success-title-warn` is fetched, carrying a title between `NAME_SNAPPY` and `NAME_MAX` characters
- **THEN** the deck SHALL surface the title step at the `warn` tier with its inline trim-suggest note — distinct from `success-long-title`'s over-`NAME_MAX` `error`-tier hard block
