...
The system SHALL provide keyboard navigation between `<MenuItem>` / `<MenuLinkItem>` rows when `<Menu>` is open. Arrow Down moves focus to the next item; Arrow Up moves focus to the previous; Home moves to the first; End moves to the last. Navigation MUST skip items with `aria-disabled="true"` and MUST wrap (Arrow Down from the last item moves to the first; Arrow Up from the first moves to the last).

#### Scenario: User navigates with arrow keys
- **WHEN** the menu is open and focus is on the first item, and the user presses Arrow Down
...
- **WHEN** a menu item has `aria-disabled="true"` and the user navigates via arrow keys
