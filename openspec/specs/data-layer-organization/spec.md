# data-layer-organization Specification
...
- **WHEN** any `lib/data/` module other than `*.actions.ts` (read modules `user.ts`, `list.ts`, `visit.ts`, `item.ts`, `purchase.ts`, `profile.ts`; internal modules `item.schema.ts`, `item.associations.ts`, `user.session.ts`) is inspected
