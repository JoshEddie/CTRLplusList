# anchor-and-run-aground Specification

## Purpose
This capability governs the two bearing-move skills that redirect an in-flight epic: `/anchor`, which dispatches on observable map state to promote, demote, charter, or re-sync map decisions while writing only GitHub issues, and `/run-aground`, the execution-layer response when a mirage strikes an `UNDER SAIL` issue — the one move that touches the working tree, staging but never signing, and owning the resume path back to the voyage.
## Requirements
