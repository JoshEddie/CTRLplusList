...
**A column that restates other rows must say which one wins.** Before adding a total, a count, or a flag that restates a join, ask what happens when it disagrees with the rows it summarises — this driver offers nothing that holds the two equal, so absent a reason the column is a rival answer with no arbiter. A value recording a past state that no current source reproduces is not this. [ADR-0016](adr/0016-claimed-units-are-summed-not-stored.md) is the worked example.
...
**Production migrations:** applied by hand — each new migration file's SQL is run against production after a snapshot, never `npm run db:migrate`. Production therefore has no Drizzle ledger; the `drizzle/meta/_journal.json` on `main` is the source of truth for which migrations have applied. The CI pre-promote gate relies on this: it seeds the ledger of a production branch from the base commit's journal and replays only the migrations added since.
